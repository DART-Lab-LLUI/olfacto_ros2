...
This experiment was created using PsychoPy3 Experiment Builder (v2024.2.5),
    on Wed 02 Apr 2025 04:42:21 PM CEST
...
expName = 'Schedule'  # from the Builder filename that created this script
...
def setupData(expInfo, dataDir=None):
    ...
    thisExp = data.ExperimentHandler(
        ...
        originPath='/home/tpmint/Documents/olfacto_ros2/scripts/Schedule_lastrun.py',
...
def run(expInfo, thisExp, win, globalClock=None, thisSession=None):
    ...
    # --- Initialize components for Routine "Odour_Program" ---
    ...
    while continueRoutine:
        ...
        if mri_trigger.status == STARTED:
            if bool(mri_trigger.keys is not None):
        ...
        if mri_trigger.status == STARTED and not waitOnFlip:
            theseKeys = mri_trigger.getKeys(keyList=['5'], ignoreKeys=["escape"], waitRelease=False)
    ...
    # --- Prepare to start Routine "Odour_Program" ---
    # create an object to store info about Routine Odour_Program
    Odour_Program = data.Routine(
        name='Odour_Program',
        components=[],
    )
    Odour_Program.status = NOT_STARTED
    continueRoutine = True
    # update component parameters for each repeat
    ...
    csv_path = '/home/tpmint/Documents/olfacto_ros2/scripts/example_schedule.csv'
    ...
    # store start times for Odour_Program
    Odour_Program.tStartRefresh = win.getFutureFlipTime(clock=globalClock)
    Odour_Program.tStart = globalClock.getTime(format='float')
    Odour_Program.status = STARTED
    thisExp.addData('Odour_Program.started', Odour_Program.tStart)
    Odour_Program.maxDuration = None
    # keep track of which components have finished
    Odour_ProgramComponents = Odour_Program.components
    for thisComponent in Odour_Program.components:
        thisComponent.tStart = None
        thisComponent.tStop = None
        thisComponent.tStartRefresh = None
        thisComponent.tStopRefresh = None
    ...
    # --- Run Routine "Odour_Program" ---
    Odour_Program.forceEnded = routineForceEnded = not continueRoutine
    while continueRoutine:
        # get current time
        t = routineTimer.getTime()
        tThisFlip = win.getFutureFlipTime(clock=routineTimer)
        ...
        # check if all components have finished
        if not continueRoutine:  # a component has requested a forced-end of Routine
            Odour_Program.forceEnded = routineForceEnded = True
            break
        continueRoutine = False  # will revert to True if at least one component still running
        for thisComponent in Odour_Program.components:
            if hasattr(thisComponent, "status") and thisComponent.status != FINISHED:
    ...
    # --- Ending Routine "Odour_Program" ---
    for thisComponent in Odour_Program.components:
        if hasattr(thisComponent, "setAutoDraw"):
            thisComponent.setAutoDraw(False)
    # store stop times for Odour_Program
    Odour_Program.tStop = globalClock.getTime(format='float')
    Odour_Program.tStopRefresh = tThisFlipGlobal
    thisExp.addData('Odour_Program.stopped', Odour_Program.tStop)
    thisExp.nextEntry()
    # the Routine "Odour_Program" was not non-slip safe, so reset the non-slip timer
    routineTimer.reset()
