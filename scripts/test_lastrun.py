...
This experiment was created using PsychoPy3 Experiment Builder (v2024.2.5),
    on Wed 02 Apr 2025 04:56:57 PM CEST
...
def setupData(expInfo, dataDir=None):
    ...
    thisExp = data.ExperimentHandler(
        ...
        originPath='/home/tpmint/Documents/olfacto_ros2/scripts/test_lastrun.py',
...
def run(expInfo, thisExp, win, globalClock=None, thisSession=None):
    ...
    # --- Initialize components for Routine "OdourProgram" ---
    ...
    while continueRoutine and routineTimer.getTime() < 10.0:
        ...
        if mri_trigger.status == STARTED:
            ...
            if tThisFlipGlobal > mri_trigger.tStartRefresh + 10-frameTolerance:
        ...
        if mri_trigger.status == STARTED and not waitOnFlip:
            theseKeys = mri_trigger.getKeys(keyList=['y','5'], ignoreKeys=["escape"], waitRelease=False)
    ...
    # --- Prepare to start Routine "OdourProgram" ---
    # create an object to store info about Routine OdourProgram
    OdourProgram = data.Routine(
        name='OdourProgram',
        components=[],
    )
    OdourProgram.status = NOT_STARTED
    continueRoutine = True
    # update component parameters for each repeat
    ...
    # store start times for OdourProgram
    OdourProgram.tStartRefresh = win.getFutureFlipTime(clock=globalClock)
    OdourProgram.tStart = globalClock.getTime(format='float')
    OdourProgram.status = STARTED
    thisExp.addData('OdourProgram.started', OdourProgram.tStart)
    OdourProgram.maxDuration = None
    # keep track of which components have finished
    OdourProgramComponents = OdourProgram.components
    for thisComponent in OdourProgram.components:
        thisComponent.tStart = None
        thisComponent.tStop = None
        thisComponent.tStartRefresh = None
        thisComponent.tStopRefresh = None
    ...
    # --- Run Routine "OdourProgram" ---
    OdourProgram.forceEnded = routineForceEnded = not continueRoutine
    while continueRoutine:
        ...
        # check if all components have finished
        if not continueRoutine:  # a component has requested a forced-end of Routine
            OdourProgram.forceEnded = routineForceEnded = True
            break
        continueRoutine = False  # will revert to True if at least one component still running
        for thisComponent in OdourProgram.components:
            if hasattr(thisComponent, "status") and thisComponent.status != FINISHED:
    ...
    # --- Ending Routine "OdourProgram" ---
    for thisComponent in OdourProgram.components:
        if hasattr(thisComponent, "setAutoDraw"):
            thisComponent.setAutoDraw(False)
    # store stop times for OdourProgram
    OdourProgram.tStop = globalClock.getTime(format='float')
    OdourProgram.tStopRefresh = tThisFlipGlobal
    thisExp.addData('OdourProgram.stopped', OdourProgram.tStop)
    thisExp.nextEntry()
    # the Routine "OdourProgram" was not non-slip safe, so reset the non-slip timer
    routineTimer.reset()
